...
hdiutil verify afternote-local-2.0.0-beta.5-darwin-arm64.dmg
```

The filename above is an example for beta.5: use the filename from the release you
...
For beta.5 these values are, in order, `2.0.0-beta.5`, `2.0.0`, `31`, and `13.3`.
For a later release, the package and build versions must match that release.
