...
describeMacos("native semantic settings", () => {
  it("defaults to local search and handles toggling, authenticated activation, malformed replies and retry", () => {
    ...
    try {
      ...
      expect(Object.values(JSON.parse(smoke.stdout.toString()))).toEqual(Array(22).fill(true));
