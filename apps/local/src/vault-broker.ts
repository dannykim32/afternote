...
export type { BrokerClientKind } from "./broker-client-kind";
export type { AuditOutcome } from "./broker-audit-reader";
...
export class VaultBrokerAuthorization {
  ...
  readonly #now: () => number;
  readonly #auditReader: BrokerAuditReader;
  ...
  constructor(options: {
    ...
    this.#auditReader = new BrokerAuditReader({
  ...
  recordAudit(
    ...
    if (noteRefs.length > MAX_AUDIT_NOTE_REFS) throw new Error("Audit note reference limit exceeded");
  ...
  inspectAudit(input: AuditPageRequest): AuditPage {
    this.#assertOpen();
    return this.#auditReader.read(input);
  }
